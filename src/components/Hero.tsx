...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Layer 2: 微妙光晕效果 - Apple 风格 */}
      <div aria-hidden="true" className="absolute inset-0 z-0 overflow-hidden">
        {/* 右上角绿色光晕 */}
        <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full bg-marrsgreen/[0.03] dark:bg-carrigreen/[0.02] blur-[120px]" />
        {/* 左下角暖色光晕 */}
        <div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] rounded-full bg-[#FF9D00]/[0.02] dark:bg-[#FF9D00]/[0.015] blur-[100px]" />
      </div>

      <div className="w-full max-w-7xl mx-auto flex flex-col-reverse lg:flex-row-reverse items-center gap-12 lg:gap-16 py-20">
        {/* 头像 - Apple 风格圆形 */}
        <div className="z-10 select-none shrink-0 lg:w-2/5">
          <div className="flex items-center justify-center mx-auto">
            <div className="relative">
              {/* 圆形渐变背景 */}
              <div className="w-48 h-48 md:w-56 md:h-56 lg:w-64 lg:h-64 rounded-full bg-gradient-to-br from-[#e8f5ee] to-[#f0f5fa] dark:from-[#0a1a12] dark:to-[#0d1a24] flex items-center justify-center shadow-lg">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 400" fill="none" shapeRendering="auto" aria-hidden="true" className="w-32 h-32 md:w-40 md:h-40 lg:w-48 lg:h-48" style={{ animation: 'avatarFloat 4s ease-in-out infinite' }}>
        ...
        <div className="z-10 relative text-center lg:text-left flex-1">
          <span className="text-marrsgreen text-sm lg:text-base font-medium dark:text-carrigreen tracking-[0.2em] uppercase">
            Hi, my name is
          </span>
          <div className="overflow-hidden mt-4">
            <h1 className="text-animation text-6xl md:text-7xl lg:text-8xl font-semibold my-2 leading-none text-apple-text dark:text-white">
              {profile.hero.name}
            </h1>
          </div>
          <div className="overflow-hidden">
            <span className="text-animation text-2xl md:text-3xl lg:text-4xl block my-4 text-marrsgreen dark:text-carrigreen font-medium">
              {profile.hero.title}
            </span>
          </div>
          <div className="mt-8 mb-10 max-w-2xl mx-auto lg:mx-0">
            <p className="text-lg leading-relaxed mb-3 text-apple-textSecondary dark:text-textdark">
              {profile.hero.subtitle}
            </p>
            <p className="text-lg leading-relaxed text-apple-textSecondary dark:text-textdark">
          ...
          <a
            role="button"
            className="inline-block bg-apple-text dark:bg-white text-white dark:text-black py-4 px-10 rounded-button text-base font-medium hover:bg-gray-800 dark:hover:bg-gray-200 transition-all duration-300 hover:-translate-y-0.5 shadow-lg hover:shadow-xl"
            href="#contact"
          >
            Contact me
          </a>
      ...
      >
        <span className="text-sm group-hover:text-marrsgreen dark:group-hover:text-carrigreen text-apple-textSecondary dark:text-textdark">
          Scroll
        </span>
        <svg
          ...
          width="24"
          height="24"
          className="dark:fill-white group-hover:fill-marrsgreen dark:group-hover:fill-carrigreen fill-apple-textSecondary dark:fill-textdark"
        >
